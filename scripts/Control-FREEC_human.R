...
################################################################################
  ...
  config <- glue(
    "[general]
  chrLenFile = /scratch/gent/vo/002/gvo00207/vsc51018/simulated_data/hg38.fa.fai      
  ploidy = 2 
  maxThreads = 16
  outputDir = {dir}  
  uniqueMatch = TRUE 
  window = 100000
  chrFiles = /scratch/gent/vo/002/gvo00207/vsc51018/chrFiles_human  
  ...
  configFile <- file.path(dir, paste0("freec_config_",base_name,".txt"))  
  ...
  ratio_data <- read.table(file.path(dir, paste0(base_name, ".bam_ratio.txt")), header = TRUE)
